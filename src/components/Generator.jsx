...
export default function Generator(props) {
  ...
  return (
   <SectionWraper id={'generate'} header={"generate your workout"} title=
   {['It\'s','Huge','o\'clock']}>
    ...
    {Object.keys(WORKOUTS).map((type,typeIndex) => {
      return(
        <button onClick={() =>
          {setMuscles([])
            setPoison(type)
        }} className={'bg-slate-950  hover:border border-blue-600 py-3 rounded-lg px-4 ' + (type === poison ? 'border border-blue-600':'border-blue-400')}
        key={typeIndex}>
    ...
    {Object.keys(SCHEMES).map((scheme,schemeIndex) => {
      return(
        <button onClick={() =>
          {setGoal(scheme)
        }} className={'bg-slate-950  hover:border border-blue-600 py-3 rounded-lg px-4 ' + (scheme === goal ? 'border border-blue-600':' ')}
        key={schemeIndex}>
